...
- **Point-in-time universe.** See caveat 3 above.
- **`docs/final_research_report.md`** still exists and is regenerated by
  `utils/generate_report.py`, which I corrected to use the direction-aware flag.
  But `docs/manuscript.md` supersedes it, and you may want to delete the old
  report rather than maintain two.
- **`docs/findings_report.html` / `.pdf`** are stale — they were generated from
  the previous numbers and I have not regenerated them. They should either be
  rebuilt or removed before this is published anywhere.
