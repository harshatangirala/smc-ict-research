...
from analytics.statistics import apply_fdr_correction, one_sample_significance
...
def rank_combinations(holding_period: int = 10) -> pd.DataFrame:
    ...
    for signal, grp in subset.groupby("signal"):
        ...
        rows.append({"combination": signal, **metrics, "p_value": sig["p_value"], "effect_size_cohens_d": sig["effect_size_cohens_d"]})

    ranking = pd.DataFrame(rows)
    if ranking.empty:
        return ranking
    fdr = apply_fdr_correction(ranking.set_index("combination")["p_value"])
    ranking = ranking.merge(fdr[["p_adjusted", "reject_null"]], left_on="combination", right_index=True, how="left")
    ranking["statistically_significant"] = ranking["reject_null"].fillna(False)
