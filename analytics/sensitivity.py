...
#: The parameters the three reformulated concepts actually read. The broad grid
#: (ob_swing_len x mss_pivot_len) never touches them, so it cannot test whether
#: those results are an artefact of the values chosen. The first run of this
#: sweep came from an ad-hoc script outside the repo; it lives here so the
#: manuscript's Table 6 can be regenerated by `python main.py sensitivity`.
TARGETED_GRID = {
    "ict.sweep_penetration_atr": [0.10, 0.25, 0.50, 1.00],
    "ict.sweep_confirm_bars": [1, 3, 6],
    "ict.gap_min_atr": [0.05, 0.10, 0.25],
}
TARGETED_SIGNALS = [
...
def run_targeted_sweep(n_tickers: int = 30) -> tuple[pd.DataFrame, pd.DataFrame]:
    """36-configuration sweep over the reformulated concepts' own parameters."""
    grid = grid_sweep(TARGETED_GRID, n_tickers=n_tickers, signals=TARGETED_SIGNALS)
    grid.to_csv(RESULTS_DIR / "sensitivity_targeted_grid.csv", index=False)
    stab = stability_summary(grid)
    stab.to_csv(RESULTS_DIR / "sensitivity_targeted_stability.csv", index=False)
    return grid, stab
